...
// Show clickable links in msgs

const msgs = document.getElementsByClassName('link-True')
for (let i = 0; i < msgs.length; i++) {
  const msg_split = msgs[i].innerHTML.split(/(\s+)/).filter(function (e) { return e.trim().length > 0; })
  let msg = ''
  for (let j = 0; j < msg_split.length; j++) {
    if (msg_split[j].startsWith('http') || msg_split[j].startsWith('http')) {
      msg += `<a href='${msg_split[j]}' target="_blank">${msg_split[j]}</a>`
    } else {
      msg += ` ${msg_split[j]}`
    }
  }
